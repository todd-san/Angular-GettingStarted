...
export class ApiService  {
  ...
  * */
  ...
  // public host: string = "http://ktek/";
  // public host: string = "http://127.0.0.1:8888";
  // public host: string = "http://kendaeng02:8080";
  ...
  * */
  ...
  /* Partial-Routes that will use the Project-Tree Menu and NOT the Spec-Tree Menu
  * */
  public project_tree_routes = ['/project/', '/phase/', '/design/'];

  /* Simply stitches the "host" and the route together
  * */
  public api(address){
  ...
  public obj_action(address, id, action){
    return address + id.toString() + action;
  }

  /* Function that identifies whether or not to render the Project-Tree Menu or the Spec-Tree Menu
  * */
  public menuTreeFromRoute(current_route){
    let project_tree = false;

    this.project_tree_routes.forEach(route =>{
      if(current_route.includes(route) || current_route === '/') {
        project_tree = true;
      }
    });
    return project_tree
